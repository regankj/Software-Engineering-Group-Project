...
def display_statistics(window, results_filename, quiz_table):
    ...
    for key in results:
        total = 0
        num = 0
        for attr in range(len(key)-1):
            total += key[attr][1]
            avg = int(total * 100 / 5)
        sam_avg.append(avg)
    avg_trend = []
    avg_list = []
    for i in range(len(sam_avg)):
        sum = 0
        avg_list.append(sam_avg[i])
        for i in range(len(avg_list)):
            sum += avg_list[i]
        avg = sum / (i+1)
        avg_trend.append(avg)

    sam_volume = []
    for i in range(len(results)):
        sam_volume.append(i+1)

    plt.plot(sam_volume, avg_trend, linewidth=5, color='b')
    plt.title("Average accuracy trend", fontsize=20, color='b')
    plt.xlabel("Sample volume", fontsize=14, color='g')
