...
def run_quiz(window, quiz_type):
    ...
    def showresult(score):
        ...
        label_score = Label(window, text="Score: {}/5".format(score), font=("Bell MT", 22, "bold"),
                            background="#ffffff")
        label_score.pack()
        label_result = Label(window, font=("Bell MT", 22, "bold"), background="#ffffff")
        label_result.pack()
    ...
    def create_radio_buttons():
        ...
        for i in range(num_of_answers_per_question):  # ie 2 if true/false, 5 if mcq
            radio_buttons_list.append(
                Radiobutton(window, text=answers_list_display[index_list[0]][i], font=("Bell MT", 16, "bold"), value=i,
                            variable=radio_var, command=select, background="#ffffff"))

    # function to initiate quiz
    def quiz_begins():
        global label_questions
        label_questions = Label(window, text=questions_list[index_list[0]], font=("Bell MT", 18, "bold"), width=500,
    ...
    lbl_title = Label(window, text="Quizardry", font=("Bell MT", 26, "bold"), background="#ffffff")

    btn_start = Button(window, text="START QUIZ", font=("Bell MT", 20, "bold"), background="green", command=start)
